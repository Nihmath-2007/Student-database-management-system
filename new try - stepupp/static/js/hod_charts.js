...
function renderHodDashboard(data) {
    if (!data) return;
...
async function initHodDashboard() {
    // 1. Instant render from sessionStorage cache if available (0ms load time)
    try {
        const cached = sessionStorage.getItem('cached_hod_analytics');
        if (cached) {
            renderHodDashboard(JSON.parse(cached));
        }
    } catch (e) {}
    ...
    try {
        ...
        if (res.ok) {
            ...
            renderHodDashboard(data);
            try {
                sessionStorage.setItem('cached_hod_analytics', JSON.stringify(data));
            } catch (e) {}
        }
    } catch (err) {
